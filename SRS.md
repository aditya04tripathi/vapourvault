...
- Anonymous file uploads via REST API (Direct and Presigned).
...
2.  **Presigned Uploads**: Generating secure URLs for direct-to-storage uploads and handling completion callbacks.
...
- **FR-02**: The system shall support a `POST /files/presign-upload` endpoint to generate presigned URLs for client-side direct uploads.
- **FR-03**: The system shall support a `POST /files/complete-upload` endpoint to mark a presigned upload as finished and trigger processing.
- **FR-04**: Upon successful upload, the system must generate a unique `fileId` (UUID).
- **FR-05**: The system must preserve the original MIME type and filename.

#### 3.1.2 File Storage & Processing

- **FR-06**: Files shall be stored in S3-compatible object storage (MinIO) under `uploads/anonymous/{fileId}/{filename}`.
- **FR-07**: A background job shall be enqueued immediately after upload (or upload completion) to process file metadata.
- **FR-08**: File metadata shall be persisted in a PostgreSQL database.

#### 3.1.3 Data Retrieval & Management

- **FR-09**: Users shall retrieve files via `GET /files/{fileId}/download`.
- **FR-10**: The retrieval endpoint shall return a presigned download URL with a limited validity period.
- **FR-11**: The system shall return metadata and processing status via `GET /files/{fileId}/status`.
- **FR-12**: The system shall allow file deletion via `DELETE /files/{fileId}`.
- **FR-13**: The system shall allow listing storage contents via `GET /files/storage/list` (Admin/Debug use).

#### 3.1.4 Data Expiration (TTL)

- **FR-14**: All files shall have a mandatory Time-To-Live (TTL) of 24 hours.
- **FR-15**: A scheduled Cron task shall execute hourly to identify files created > 24 hours ago.
- **FR-16**: Expired files must be permanently deleted from both the database and object storage.
...
- **NFR-04**: Access controlled by the secrecy of the UUID `fileId`.
- **NFR-05**: All storage URLs must be signed (Presigned URLs).

#### 3.2.3 Reliability & Availability

- **NFR-06**: ACID compliance for metadata consistency via PostgreSQL.
